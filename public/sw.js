const CACHE_NAME = 'smart-salon-v23';
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      console.log('[sw.js] Pre-caching core assets');
  ...
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((name) => {
          if (name !== CACHE_NAME) {
            console.log('[sw.js] Clearing old cache:', name);
...
self.addEventListener('push', (event) => {
  let data = { title: 'Smart Salon', body: 'আপনার একটি নতুন আপডেট আছে।' };
  if (event.data) {
    try {
